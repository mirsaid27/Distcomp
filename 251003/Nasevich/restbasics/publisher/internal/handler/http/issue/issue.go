...
import (
	...
	issueModel "github.com/Khmelov/Distcomp/251003/Nasevich/restbasics/publisher/internal/model"
	issueDbModel "github.com/Khmelov/Distcomp/251003/Nasevich/restbasics/publisher/internal/storage/psql/issue"
...
func (i Issue) InitRoutes(r *mux.Router) {
	r.HandleFunc("/issues", i.getIssuesList).Methods(http.MethodGet)
	r.HandleFunc("/issues", i.createIssue).Methods(http.MethodPost)
	r.HandleFunc("/issues/{id}", i.getIssueByID).Methods(http.MethodGet)
	r.HandleFunc("/issues/{id}", i.deleteIssueByID).Methods(http.MethodDelete)
	r.HandleFunc("/issues", i.updateIssueByID).Methods(http.MethodPut)
}
