...
import (
	...
	messageService "github.com/Khmelov/Distcomp/251003/Nasevich/restbasics/publisher/internal/service/message"
...
func (h *MessageHandler) InitRoutes(r *mux.Router) {
	r.HandleFunc("/messages", h.getMessagesList).Methods(http.MethodGet)
	r.HandleFunc("/messages/{id}", h.getMessageByID).Methods(http.MethodGet)
	r.HandleFunc("/messages", h.createMessage).Methods(http.MethodPost)
	r.HandleFunc("/messages/{id}", h.deleteMessageByID).Methods(http.MethodDelete)
	r.HandleFunc("/messages", h.updateMessageByID).Methods(http.MethodPut)
}
