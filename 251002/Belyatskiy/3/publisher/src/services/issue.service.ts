...
export class ArticleService {
    ...
    // Пример вызова Message API
    async addMessage(articleId: number, messageDto: MessageRequestTo) {
        messageDto.articleId = articleId;
        const response = await axios.post(this.discussionApi, messageDto);
        return response.data;
    }

    async findAll(page: number = 1, limit: number = 10, sortBy: string = 'id', order: 'ASC' | 'DESC' = 'ASC'): Promise<ArticleResponseTo[]> {
    ...
    async findById(id: number): Promise<ArticleResponseTo | undefined> {
