...
{
    private readonly KafkaConsumer<Ignore, MessageResponse> _consumer;
    ...
    )
    {
        ...
        _logger = logger;
        var handler = serviceProvider.GetRequiredService<MessageResponseHandler>();

        var serializer = new SystemTextJsonSerializer<MessageResponse>(
            new JsonSerializerOptions { Converters = { new JsonStringEnumConverter() } }
        );

        _consumer = new KafkaConsumer<Ignore, MessageResponse>(
