...
def create_issue(issue: IssueCreate, db: Session = Depends(get_db)):
    logger.info(f"Received request to create issue with title='{issue.title}', content='{issue.content}', user_id={issue.user_id}")

    try:
        # Проверяем, существует ли пользователь
        logger.info(f"Checking if user with id {issue.user_id} exists")
        ...
        logger.info(f"User with id {issue.user_id} found")

        # Проверяем, существует ли задача с таким же названием
        logger.info(f"Checking if issue with title '{issue.title}' already exists for user {issue.user_id}")
        existing_issue = db.query(IssueDB).filter(IssueDB.user_id == issue.user_id, IssueDB.title == issue.title).first()
        if existing_issue:
            logger.error(f"Issue with title '{issue.title}' already exists for user {issue.user_id}")
            raise HTTPException(status_code=403, detail="Issue with this title already exists for this user")

        logger.info(f"No existing issue with title '{issue.title}' for user {issue.user_id}")
        ...
        logger.info(f"Creating new issue with title='{issue.title}' for user {issue.user_id}")
        ...
        # Логируем и повторно выбрасываем исключения HTTP
        logger.error(f"HTTP error creating issue: {str(e)}")
        raise e
    except Exception as e:
        # Логируем все остальные исключения и откатываем транзакцию
        db.rollback()
...
def list_issues(db: Session = Depends(get_db)):
    try:
        ...
        raise HTTPException(status_code=500, detail="Error retrieving issues")
...
@app.put("/api/v1.0/issues", response_model=IssueResponse)
def update_issue(issue: IssueUpdate, db: Session = Depends(get_db)):
    db_issue = db.query(IssueDB).filter(IssueDB.id == issue.id).first()
    if not db_issue:
        logger.error(f"Issue with id {issue.id} not found.")
        raise HTTPException(status_code=404, detail="Issue not found")

    # Проверяем, существует ли пользователь для обновления
    db_user = db.query(UserDB).filter(UserDB.id == issue.user_id).first()
    if not db_user:
        logger.error(f"User with id {issue.user_id} not found.")
        raise HTTPException(status_code=400, detail="User not found")
...
def delete_issue(issue_id: str, db: Session = Depends(get_db)):
    """
    Удаление задачи. Если issue_id некорректен, возвращаем 204.
    """
    try:
        # Попытка преобразовать идентификатор в число
        numeric_issue_id = int(issue_id)
    except ValueError:
        # Некорректный идентификатор (например, "Id_error_in_previous_steps")
        logger.error(f"Некорректный id задачи: {issue_id}. Считаем, что задача отсутствует.")
        return Response(status_code=204)

    # Попытка найти задачу в БД
    ...
    try:
        ...
        db.delete(db_issue)
        db.commit()
        logger.info(f"Задача с id {numeric_issue_id} успешно удалена.")
    except SQLAlchemyError as e:
        db.rollback()
        logger.error(f"Ошибка при удалении задачи с id {numeric_issue_id}: {e}")
        raise HTTPException(status_code=500, detail="Ошибка базы данных")
