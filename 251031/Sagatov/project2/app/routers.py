...
def get_all_writers(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    return WriterService(db).get_all(skip, limit)

# ... остальные endpoints с аналогичными изменениями
