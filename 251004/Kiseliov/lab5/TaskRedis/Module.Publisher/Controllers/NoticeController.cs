...
[ApiVersion("1.0")]
[ApiController]
public class NoticeController(INoticeService service, IOutputCacheStore cacheStore) : ControllerBase
{
    [HttpGet("{id:long}")]
    ...
    public async Task<ActionResult<NoticeResponseTo>> CreateNotice(CreateNoticeRequestTo createNoticeRequestTo)
    {
        ...
        await cacheStore.EvictByTagAsync(CacheTags.Notices, HttpContext.RequestAborted);
    ...
    {
        var country = GetCountryFromRequest();
        NoticeResponseTo updateNotice = await service.UpdateNotice(updateNoticeRequestTo, country);
        await cacheStore.EvictByTagAsync(CacheTags.Notices, HttpContext.RequestAborted);
